...
def get_boundary(mask):
    """
    Extract boundary from a binary mask.
    Args:
        mask: Binary mask, shape (H, W), (B, H, W), or with extra dimensions like (1, H, W).
    Returns:
        Boundary mask with the same shape as input (H, W) or (B, H, W).
    """
    input_is_tensor = isinstance(mask, torch.Tensor)
    if input_is_tensor:
    ...
    # Check dimensions and process
    if mask.ndim == 3:  # Batch input (B, H, W)
        boundaries = np.array([find_boundaries(m, mode="outer") for m in mask])
    elif mask.ndim == 2:  # Single image input (H, W)
        boundaries = find_boundaries(mask, mode="outer")
    else:
        raise ValueError(f"Invalid mask dimensions: expected 2D (H, W) or 3D (B, H, W), but got {mask.shape}")
